...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, Keymapper status) const {
 ...
 /* сделайте отдельный метод FindTopDocuments с одним параметром
  * и в нем передавайте лямбду с провекой статуса в вариант метода с двумя параметрами*/
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus stat = DocumentStatus::ACTUAL) const {
    	return FindTopDocuments(raw_query, [stat](int document_id, DocumentStatus status, int rating) { return stat == status; });
    }
